...
| T-048 | **Add "full detail" summary mode (on-demand)** | High | ✅ Implemented 2026-07-13, Thai transcript fallback fixed and live-verified 2026-07-23. Full Detail now expands Thai/original caption tracks (`th`, `th-orig`) before English fallback, and stale transcript-failure stubs are regenerated instead of treated as completed reports. Mandy's real `research_job` video `kL1819NMiKo` now generates a valid Thai Full Detail report from `/assets/manage`; existing report/audio/digest remain untouched. Original scope: `--detailed` mode compresses instructional/tutorial videos into thematic bullets, while Full Detail preserves the sequential content. Shipped as an **on-demand** action on `/assets/manage` (`/api/assets/generate-full-detail`), available to all topics, and writes a sibling `{date}-v{N}-full-detail.md` file. Verification: real local API flow returned HTTP 200/status `ok`, output contains `### 📝 Full Summary (Thai)`, is not a transcript-failure stub, and 15 focused regression tests pass. See `docs/FULL-DETAIL-SUMMARY-MODE-DESIGN-2026-07-13.md` for full design history. |
