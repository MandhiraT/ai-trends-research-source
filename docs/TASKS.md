...
| T-048 | **Add "full detail" summary mode (on-demand)** | High | ✅ Implemented 2026-07-13, commits `7dc4196`/`7b10177`. NATEHERK-only gate lifted to all topics same day per Mandy's instruction, plus fixed a related bug (B-009) that made on-demand reports invisible to the whole Assets system. Mandy reported `--detailed` mode compresses instructional/tutorial videos into 4-7 thematic bullets, losing individual steps (example: 42-command camera-movement tutorial → only ~15 commands surfaced across 5 themes). Shipped as an **on-demand** action on `/assets/manage` (`/api/assets/generate-full-detail`), gated to NATEHERK only, writes a sibling `{date}-v{N}-full-detail.md` file — existing report/audio/digest untouched. Same pass also addressed Mandy's follow-up concern about page clutter: consolidated Quick Script/Long Narration/Full Detail into 2 dropdowns (was 4+ separate buttons), renamed "Full Script"→"Quick Script" and "Deep Dive"→"Long Narration" to clarify both work from the report (not the transcript), added an overwrite warning for the 6 topics with automated daily audio, and cut the unused ~150-word "short script" variant. Verified live against a real NATEHERK video — 22 concrete steps captured. See `docs/FULL-DETAIL-SUMMARY-MODE-DESIGN-2026-07-13.md` for full design history. |
| T-049 | **Full Detail → git-push confirmation + send to FAW/MJS** | Medium | 🔜 Design done 2026-07-14, awaiting Mandy's decisions — not implemented. Mandy asked for Full Detail reports to also push to git like a normal report, and to be sendable to FAW and MJS. Found: (1) git push already happens today with zero new code — the publisher does a blanket `rglob("*")` sweep of the whole `reports/` tree with no filename filtering, so a full-detail sibling file is already included in every morning's push; (2) MJS's existing brief bridge (`ats_to_mjs_bridge.py`) is structurally wrong for Full Detail — it looks for `## Video N:` headers (absent) and falls back to `report_text[:4000]`, silently truncating ~80% of a real full-detail file's content, defeating the whole point of the feature; found a related **independent bug** (not asked for, flagging separately): MJS's `/ats` Source Browser sweeps every `.md` with no date validation, so full-detail files already show up today as bogus fake source cards, and clicking Create Brief on one hits the same truncation bug; (3) FAW has a real, working ATS integration (`ats_faw_bridge.py`/`ats_source_brief.py`) but it's entirely keyed to a separate asset-JSON file + `## Video N:` + specific Thai section markers, none of which a Full Detail file has — sending one through today's bridge would hard-fail or produce a near-empty draft. See `docs/FULL-DETAIL-GIT-PUBLISH-AND-FAW-MJS-SEND-DESIGN-2026-07-14.md` for full analysis + 4 open questions (what FAW should actually produce from it; brief-review vs. direct MJS seed; entry point on `/assets/manage`; whether to bundle the fake-source-card bugfix). |
...
| B-009 | On-demand reports invisible to entire Assets system | High | ✅ Fixed 2026-07-13 | Mandy noticed "on demand" never appeared as a topic in Assets. Root cause: on-demand reports are named `{date}_{HHMM}_{slug}.md` (unique per run, since several can exist per day) but every date-validation regex + `find_reports()` only accepted plain `{date}.md` — structurally excluding on-demand reports from asset generation, `/assets`, `/assets/manage`, and Full Detail. Separately, `build_asset_from_report()` also scattered each on-demand report's asset into a different topic folder per run (using the free-text "Topic:" metadata, e.g. "Claude", instead of the stable "research_job" folder name). Fixed both (`dashboard/app.py` + `generate_content_assets.py`, commit `7b10177`); verified live against the actual on-demand report that triggered T-048 — now groups correctly under "research_job". All 43 regular cron jobs use plain `{date}.md` and were never affected. |
| B-010 | Full Detail "View" dropdown selectable before generation → 404 | Medium | ✅ Fixed 2026-07-13 | Mandy hit "View → Full Detail" on a report where it hadn't been generated yet and got a raw 404. Root cause: the View dropdown's Full Detail `<option>` reused the exact same markup as the Generate dropdown's, which is never disabled — unlike Quick Script/Long Narration, whose View options already correctly disable + label "(not generated)" until a file exists. Fixed by splitting into `fd_gen_option` (Generate, always enabled) and `fd_view_option` (View, disabled + "(not generated)" until the file exists) (`dashboard/app.py`, commit `16e724f`); verified live against the exact report Mandy hit this on — generated Full Detail for it, confirmed `/view/full-detail` now returns 200. |
| B-011 | New cron jobs missing from Telegram daily summary ("Claude Code Fable 5" + "Sabrina") | Medium | ✅ Fixed 2026-07-14 | Mandy asked why the "Claude Code Fable 5" job she added 2026-07-13 never showed up in the 09:30 Thai daily-summary Telegram message, even though it ran fine (07:35 batch, report saved, 2 videos today). Root cause: `scripts/ai_trends_daily_summary_thai.py` keeps its own hardcoded `TOPICS` list (label, job id, github path) completely separate from `config/research_jobs.json`, and it's never updated when a new job is added — so the summary script has no entry to check for it at all. Auditing all 43 daily-cron jobs against `TOPICS` found a second, longer-standing instance of the same bug: "Sabrina" (added 2026-06-21) has been producing reports daily and was silently absent from every summary since. Fixed by adding both entries to `TOPICS` (`scripts/ai_trends_daily_summary_thai.py`); verified by calling `build_status()` directly for 2026-07-14 (no live Telegram send, to avoid a duplicate real notification) — now correctly returns "✅ CC Fable 5 — 2 videos" and "✅ Sabrina — 2 videos", and a full audit confirms all 43 daily-cron jobs are now represented in `TOPICS`. |
